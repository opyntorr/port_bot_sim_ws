...
def generate_launch_description():
    ...
    # NOTA: este launch corre el robot REAL (Yahboom: yahboom_base + ydlidar). El bloque
    # de Gazebo (gazebo, puente, spawner, seq_*, odom_forwarder) NO va en el return -> es
    # codigo muerto / espejo. El spawn sim se migro a 'jetauto' por consistencia, pero la
    # parte de hardware sigue siendo Yahboom (no hay drivers reales del JetAuto en el repo).
    puente = Node(
    ...
    # Bloque sim espejo; NO va en el return -> inactivo.
    spawner = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=[
    ...
    # =========================================================
    # LANZAR TODO (robot REAL Yahboom + pipeline de mapeo SLAM)
    # =========================================================
    return LaunchDescription([
        ...
        # --- Pipeline de mapeo (teleop + filtro lidar + SLAM + RViz) ---
        joy_node,
        teleop,
        filtro_lidar_node,
        slam_toolbox_launch,
