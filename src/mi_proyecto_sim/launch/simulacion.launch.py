...
def generate_launch_description():
    ...
    set_env = SetEnvironmentVariable(
        ...
        value=f"{models_dir}:{pkg_sim}:{tello_models_dir}"  # pkg_sim: mallas del JetAuto
    )
    ...
    puente = Node(
        ...
        arguments=[
            ...
            # Sensores del carrito (/scan, /cam_1/*, /imu/data_raw) y odom/TF -> jetauto_bringup.
            # /cmd_vel del carrito lo consume jetauto_chassis_sim (lado ROS), no se puentea a gz.
        ],
        output='screen'
    )
    ...
    # El robot (RSP + spawn + controladores + chassis + IMU + EKF) lo aporta el include
    # `jetauto`. SLAM/cartographer/publicador_tfs/RRT arrancan diferidos por
    # rrt_y_slam_handler tras convertir el mapa del dron. La consola web (rosbridge/
    # foxglove/web_video) queda comentada: esos paquetes no estan instalados en el contenedor.
    return LaunchDescription([
        slam_backend_arg,   # IMPRESCINDIBLE: sin esto LaunchConfiguration('slam_backend') queda
                            # vacio, is_slam_toolbox/is_cartographer dan False y SLAM +
                            # publicador_tfs nunca arrancan (no hay map->odom -> RViz/RRT cuelgan).
        limpiar_artefactos,
        set_env,
        plugin_env,
        gui_fov_env,
