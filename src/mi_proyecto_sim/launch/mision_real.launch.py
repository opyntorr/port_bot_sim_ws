...
def generate_launch_description():
    ...
    # NOTA: este launch corre el robot REAL (Yahboom: yahboom_base + ydlidar + usb_cam).
    # El bloque de Gazebo (gazebo, puente, spawner, seq_*) NO va en el return -> es
    # codigo muerto / espejo de visualizacion. El spawn sim se migro a 'jetauto' por
    # consistencia, pero la parte de hardware sigue siendo Yahboom (no hay drivers
    # reales del JetAuto en el repo).
    puente = Node(
    ...
    robot_state_publisher = Node(
        ...
        parameters=[{
            'use_sim_time': False,  # <--- Sincronizado
            'robot_description': ParameterValue(Command(['xacro ', xacro_file, ' use_gazebo:=true']), value_type=str)
        }]
    )

    # 6. Gazebo Spawner (bloque sim espejo; NO va en el return -> inactivo)
    spawner = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=[
    ...
    rviz_node = Node(
        ...
        parameters=[{'use_sim_time': False}]  # <--- Sincronizado
    )
    ...
    rosbridge_server = Node(
        ...
        parameters=[{'use_sim_time': False, 'port': 9090}],
        output='screen',
    )
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            remappings=[
                ...
                ('/image_raw/compressed', '/cam_1/image/compressed')
            ]
        ),
    ])
