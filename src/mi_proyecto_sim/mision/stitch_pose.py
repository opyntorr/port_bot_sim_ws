...
def main():
    ...
    # Undistort images using Tello calibration
    calib_file = "/home/opyntorr/agv_uav_project_jetauto/src/demo_tello_sim/camera_calibration/calibration_output/tello_calibration.npz"
    if Path(calib_file).exists():
        npz = np.load(calib_file)
        K_mat, D_mat = npz['K'], npz['D']
        for t in tiles:
            t.img = cv2.undistort(t.img, K_mat, D_mat)
        fy = float(K_mat[1, 1])
        sample_h = tiles[0].img_h
        cfg.fov_v_deg = math.degrees(2.0 * math.atan(sample_h / (2.0 * fy)))
        print(f"[stitch_pose] Undistorted {len(tiles)} tiles. fov_v_deg={cfg.fov_v_deg:.2f}°")
    ...
    is_sim = "sim" in getattr(args, "camera", "").lower() or "sim" in str(args.input).lower()
